...
- [x] 7.1 修改 `DialogueAgent`，candidate 消息后触发分类
- [x] 7.2 分类结果写入对应模块文档
- [x] 7.3 MD 变更触发解析 → 生成事件日志
- [x] 7.4 添加 `module_document.updated` 事件类型

## 8. 投影器更新

- [x] 8.1 修改 `ProjectorService` 处理 `module_document.updated` 事件
- [x] 8.2 实现从 MD 文档重建 modules 状态
- [x] 8.3 更新 `story_state.json` 结构

## 9. 前端 - 模块画布 UI

- [x] 9.1 创建 `ModulesView` 组件
- [x] 9.2 实现五模块卡片展示
- [x] 9.3 实现模块文档编辑器（Markdown 编辑）
- [x] 9.4 实现编辑锁获取/释放逻辑
- [x] 9.5 实现系统添加队列指示器

## 10. 数据迁移

- [x] 10.1 创建迁移脚本：扁平列表 → MD 文档
- [x] 10.2 测试迁移脚本
- [x] 10.3 验证迁移后数据完整性

## 11. 测试与验证

- [x] 11.1 单元测试：MD 解析器和渲染器
- [x] 11.2 单元测试：分类 API
- [x] 11.3 单元测试：乐观锁机制
- [x] 11.4 集成测试：对话流程 → 分类 → 写入 MD
- [ ] 11.5 端到端测试：前端编辑 → 锁 → 保存 → 队列处理
- [x] 11.6 全量测试通过（185 passed）
